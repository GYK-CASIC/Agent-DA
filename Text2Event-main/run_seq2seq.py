...
# 数据类，用于存储模型相关的参数。这些参数包括预训练模型路径、配置名称、分词器名称、缓存目录等
@dataclass
...
# 用于存储数据训练相关的参数。包括任务类型、数据集名称是否忽略填充标记的损失等等
@dataclass
...
# 定义两个字典，分别用于映射摘要任务和事件抽取任务的数据集列名
...
# 脚本入口，首先通过HfArgumentParser解析命令行参数或文件中的参数，分别生成模型参数、数据参数、训练参数
def main():
    ...
    # Log on each process the small summary:每个进程上记录一些训练的基本信息
    logger.warning(
    ...
    def preprocess_function(examples):
        ...
        return model_inputs
    # 对训练、验证和测试数据集进行预处理，并根据需要选择最大样本数
    if training_args.do_train:
    ...
    # 根据配置创建数据整理器，用于将数据批处理时对其进行适当的填充
    # Data collator
    label_pad_token_id = - \
    ...
    if training_args.do_train:
        ...
        checkpoint = None
        # 如果设置了do_train 参数，则调用训练器得到train方法进行训练，并保存模型和训练结果
    ...
    # 如果设置了 do_eval 参数，则调用训练器的 evaluate 和 predict 方法进行验证，并保存验证结果和预测结果。
    # Evaluation
    results = {}
    if training_args.do_eval:
        ...
        if trainer.is_world_process_zero():
            ...
            if training_args.predict_with_generate:
                ...
                with open(output_test_preds_file, "w") as writer:
                    writer.write("\n".join(eval_preds))
    # 如果设置了 do_predict 参数，则调用训练器的 predict 方法进行测试，并保存测试结果和预测结果。
    if training_args.do_predict:
        logger.info("*** Test ***")
