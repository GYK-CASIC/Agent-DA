...
def main():
    ...
    def compute_metrics(eval_preds):
        preds, labels = eval_preds # 将预测结果和标签传递给 
        if isinstance(preds, tuple):
            preds = preds[0]# 如果预测结果是元组，则取第一个元素
    ...
    if training_args.do_predict:
        logger.info("*** Test ***")
        # 调用模型的generate方法，设置return_dict=True来确保logits被返回
        test_results = trainer.predict(
            test_dataset,
            metric_key_prefix="test",
            max_length=data_args.val_max_target_length,
            num_beams=data_args.num_beams,
            #output_scores=True# 这个是？
        )
        # print("test_result",test_results,test_results.shape)
        print("test_dataset",test_dataset)
        all_logits = []
        # print("test_dataset.input_ids",test_dataset["input_ids"])
        for example in test_dataset:
            ...
            # print("labels_token_prob_list:",labels_token_prob_list)
        # 计算生成每个label的概率,labels_token_prob_list中所有token的概率相乘
            # labels_prob_list = torch.prod(labels_token_prob_list, dim=-1)
            # print(labels_prob_list)
            print("loss",loss)
            text_prob=torch.exp(-loss)*(len(labels))
            text_prob = round(text_prob.item(), 4)  # 保留四位小数
            print("text_prob",text_prob)
            
            all_logits.append(text_prob)
        print("all_logits:",all_logits)
        # all_logits = torch.cat(text_prob, dim=0)
        # print("all_logits:",all_logits)
        ...
        if trainer.is_world_process_zero():
            ...
            if training_args.predict_with_generate:
                ...
                # print("test_preds.predictions:", test_results.predictions)
